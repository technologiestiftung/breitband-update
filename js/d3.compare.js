...
function d3_compare(){
	...
	function compare(sel){
		...
		d3.csv("data/all.csv").get(function(error, csv) {
			...
			bezirke.forEach(function(d){
				...
				d.mean = d3.mean(d.values.filter(function(d) { return d.jahr === 2017 && d.type === 'leitung16'; }), function(d){ return d.value; }).toFixed(1) + '% | ' + d3.mean(d.values.filter(function(d) { return d.jahr === 2017 && d.type === 'leitung50'; }), function(d){ return d.value; }).toFixed(1)+'%';
			});
	...
	compare.resize = function(){
		...
		xAxis = d3.svg.axis()
			.scale(x)
			.orient("bottom")
			.tickValues([2011,2012,2013,2015,2017])
		...
		[bezirkeG,bezirkeA].forEach(function(b,index,array){
			b.selectAll(".ortsteil")
				...
				.on("mouseenter", function(d1){
					...
					var rect = d3.select('#'+key).node().getBoundingClientRect();
					var o = {top: rect.top + window.scrollY, left: rect.left + window.scrollX};
					var tx = o.left + x(d1.values[3].jahr) + margin.left, ty;
					if(!state.activeMenu){
						ty = o.top + y(d1.values[3].mean) + margin.top + 3;
					}else{
						ty = o.top + y(d1.values[3].value) + margin.top + 3;
					}
